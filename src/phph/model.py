...
class model:
    ...
    def __init__(
        ...
        if (
            np.any(np.isnan(servers))
            or not self.feasibleParameters(arrivalInitDistribution, arrivalGenerator)
            or not self.feasibleParameters(serviceInitDistribution, serviceGenerator)
        ):
    ...
    def meanQueueLength(self):
        ...
        if self.feasibleParam:
            ...
            meanQueue = np.sum(
                np.matmul(
                    self.localStateDist(self.queue.servers + 1),
                    np.linalg.matrix_power(
                        np.subtract(np.identity(l), self.subMats.neutsMat), -2
                    ),
                )
            )
            return meanQueue
    ...
    def meanOccupancy(self):
        ...
        """
        if self.feasibleParam:
            meanOcc = self.meanQueueLength() + (
                self.queue.meanInterServiceTime() / self.queue.meanInterArrivalTime()
            )
            return meanOcc
        else:
            return float("nan")
    ...
    def probWait(self, type="actual"):
        ...
        if self.feasibleParam:
            if type == "actual":  # observed by the customers
                ...
                l = len(self.ls.stateSpace)
                return 1 - np.sum(
                    self.boundaryProb[0, 0 : (self.boundaryProb.shape[1] - l)]
                )
        else:
            return float("nan")
    ...
    def __probPhase(self, i):
        ...
        pk = beta + np.matmul(
            np.matmul(
                self.localStateDist(self.queue.servers),
                np.linalg.inv(
                    np.subtract(
                        np.identity(self.subMats.neutsMat.shape[1]),
                        self.subMats.neutsMat,
                    )
                ),
            ),
            lk,
    ...
    def localStateDist(self, k):
        ...
        if k >= self.queue.servers:
            l = len(self.ls.stateSpace)
            xc = self.boundaryProb[
                0, (self.boundaryProb.shape[1] - l) : self.boundaryProb.shape[1]
            ]
            xk = np.matmul(
                xc,
                np.linalg.matrix_power(self.subMats.neutsMat, (k - self.queue.servers)),
            )
        elif k > 0:
    ...
    def __solveBoundary(self, method="gauss"):
        ...
        # normalize
        scaler = np.sum(x[0, 0 : x.shape[1] - self.subMats.localMat.shape[0]]) + np.sum(
            np.matmul(
                x[0, x.shape[1] - self.subMats.localMat.shape[0] : x.shape[1]],
                np.linalg.inv(
                    np.subtract(
                        np.identity(self.subMats.neutsMat.shape[1]),
                        self.subMats.neutsMat,
    ...
    def __createBoundaryMatrix(self):
        ...
        for i in range(self.queue.servers + 1):
            if i == 0:
                ...
                boundMat[
                    0 : fMat.shape[0], lMat.shape[1] : (lMat.shape[1] + fMat.shape[1])
                ] = fMat
                dim_i += fMat.shape[0]
                ...
                boundMat[
                    dim_i : (dim_i + bMat.shape[0]), dim_j : (dim_j + bMat.shape[1])
                ] = bMat
                boundMat[
                    dim_i : (dim_i + lMat.shape[0]),
                    (dim_j + bMat.shape[1]) : (dim_j + bMat.shape[1] + lMat.shape[1]),
                ] = lMat
            else:
                fMat = self.subMats.createForwardInhomMatrix(i, i + 1)
                bMat = self.subMats.createBackwardInhomMatrix(i, i - 1)
                ...
                boundMat[
                    dim_i : (dim_i + fMat.shape[0]),
                    (dim_j + bMat.shape[1] + lMat.shape[1]) : (
                        dim_j + bMat.shape[1] + lMat.shape[1] + fMat.shape[1]
                    ),
                ] = fMat
    ...
    def __actualWaitDist(self, t):
        ...
        queue_s = Queue(
            gamma_s,
            T_s,
            self.queue.serviceInitDistribution,
            self.queue.serviceGenerator,
            self.queue.servers,
        )
        ...
        while (pw - cm) > self.eps:
            ct += 1
            locStateDist = np.matmul(
                xc,
                np.linalg.matrix_power(
                    self.subMats.neutsMat, (ct - self.queue.servers)
                ),
            )

            pr = np.zeros((1, nSerStates))
            for i in range(self.queue.nPhasesArrival()):
                prvec = self.__vectorProbServiceStatePhase(
                    i, nSerStates, state, locStateDist
                )
                for j in range(nSerStates):
    ...
    def __fixMatrix(self, mat):
        ...
        if isinstance(mat, np.ndarray) and mat.ndim == 2:
            ...
            isinstance(mat, list)
            and len(mat) >= 1
            and not isinstance(mat[0], int)
            and not isinstance(mat[0], float)
            and len(mat[0]) >= 1
        ):
    ...
    def feasibleParameters(self, dist, gen):
        ...
        if gen.shape[0] != gen.shape[1]:
            print(
                "Error: In the PH generators, the number of rows must equal the number of columns."
            )
            return False
        elif dist.shape[1] != gen.shape[0]:
            print(
                "Error: The number of elements in the initial distribution must equal the number of rows and columns in the corresponding PH generator."
            )
            return False
